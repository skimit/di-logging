.PHONY: help clean list-containers start-dependencies stop-dependencies run enter-container show-container-logs test coverage sync-requirements debug

PROJECT_NAME=di_logging
FILES_TO_CHECK_FOR_CHANGES=pyproject.toml docker/docker-compose.yml docker/Dev.Dockerfile docker/Dockerfile
...
clean:	### Removes cache, compiled files, coverage results and other files.
	...
	rm -rf $(PROJECT_NAME).egg-info/ build/ dist/ .cache/
...
run-task:	### Executes a custom Poetry task. Run 'make list-tasks' to see the available tasks.
	@poetry run task $(RUN_ARGS)
...
debug:	### Runs the project tests in debug mode
ifeq ($(strip $(PROJECT_INSTANCES)),0)
	$(call ANNOUNCEMENT,Error running $(PROJECT_NAME) tests. Are the dependencies up and running?)
else
	$(call ANNOUNCEMENT,Preparing testing environment)
	$(call DOCKER_EXEC,$(PROJECT_NAME),"poetry run task debug $(EXTRA_ARGUMENTS)")
endif
